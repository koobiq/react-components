## API Report File for "koobiq-react"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts
import type { AriaListBoxProps } from '@koobiq/react-primitives';
import type { ComponentPropsWithRef } from 'react';
import type { ComponentRef } from 'react';
import type { CSSProperties } from 'react';
import type { DataAttributeProps } from '@koobiq/react-core';
import type { ElementType } from 'react';
import type { ExtendableComponentPropsWithRef } from '@koobiq/react-core';
import type { ExtendableProps } from '@koobiq/react-core';
import { ForwardRefExoticComponent } from 'react';
import type { ItemProps as ItemProps_2 } from '@koobiq/react-core';
import { JSX } from 'react/jsx-runtime';
import { ListState } from '@koobiq/react-primitives';
import { PolyForwardComponent } from '@koobiq/react-core';
import type { ReactElement } from 'react';
import type { ReactNode } from 'react';
import type { Ref } from 'react';
import { RefAttributes } from 'react';
import type { SectionProps as SectionProps_2 } from '@koobiq/react-core';

// Warning: (ae-forgotten-export) The symbol "CompoundedComponent" needs to be exported by the entry point index.d.ts
//
// @public
export const List: CompoundedComponent;

// @public (undocumented)
export type ListComponent = <T>(props: ListProps<T>) => ReactElement | null;

// @public (undocumented)
export function ListInner<T extends object>(
  props: ListInnerProps<T>
): JSX.Element;

// @public (undocumented)
export type ListInnerProps<T extends object> = {
  state: ListState<T>;
  listRef?: Ref<HTMLUListElement>;
  children?: ListProps<T>['children'];
} & Omit<ListProps<T>, 'ref' | 'children'>;

// Warning: (ae-forgotten-export) The symbol "ItemProps" needs to be exported by the entry point index.d.ts
//
// @public (undocumented)
export function ListItem<T>(_props: ItemProps<T>): null;

// @public (undocumented)
export namespace ListItem {
  var // (undocumented)
    getCollectionNode: unknown;
}

// @public (undocumented)
export const ListItemText: ForwardRefExoticComponent<
  Omit<ListItemTextProps, 'ref'> & RefAttributes<HTMLSpanElement>
>;

// @public (undocumented)
export type ListItemTextProps = ExtendableComponentPropsWithRef<
  {
    children?: ReactNode;
    caption?: ReactNode;
    autoWidth?: boolean;
    slotProps?: {
      text?: TypographyProps;
      caption?: TypographyProps;
    };
  } & DataAttributeProps,
  'span'
>;

// @public (undocumented)
export type ListItemTextRef = ComponentRef<'span'>;

// @public (undocumented)
export type ListProps<T> = ExtendableProps<
  {
    className?: string;
    style?: CSSProperties;
    ref?: Ref<HTMLElement>;
    slotProps?: {
      label?: TypographyProps;
      list?: ComponentPropsWithRef<'div'>;
    };
    isPadded?: boolean;
    isLoading?: boolean;
    onLoadMore?: () => void;
    noItemsText?: ReactNode;
    loadingText?: ReactNode;
    children?: AriaListBoxProps<T>['children'];
  },
  Omit<AriaListBoxProps<T>, 'children'>
>;

// @public (undocumented)
export type ListRef = ComponentRef<'ul'>;

// Warning: (ae-forgotten-export) The symbol "SectionProps" needs to be exported by the entry point index.d.ts
//
// @public (undocumented)
export function ListSection<T>(_props: SectionProps<T>): null;

// @public (undocumented)
export namespace ListSection {
  var // (undocumented)
    getCollectionNode: unknown;
}

// Warnings were encountered during analysis:
//
// packages/components/dist/components/List/components/ListItemText/types.d.ts:14:9 - (ae-forgotten-export) The symbol "TypographyProps" needs to be exported by the entry point index.d.ts

// (No @packageDocumentation comment for this package)
```
