## API Report File for "koobiq-react"

> Do not edit this file. It is a report generated by [API Extractor](https://api-extractor.com/).

```ts
import type { AriaDisclosureProps } from '@koobiq/react-primitives';
import type { ButtonProps } from '@koobiq/react-primitives';
import type { ComponentPropsWithRef } from 'react';
import type { ComponentRef } from 'react';
import { Context } from 'react';
import type { DisclosureGroupProps } from '@koobiq/react-primitives';
import type { DisclosureGroupState } from '@koobiq/react-primitives';
import type { DisclosureState } from '@koobiq/react-primitives';
import { ElementType } from 'react';
import type { ExtendableComponentPropsWithRef } from '@koobiq/react-core';
import { ForwardRefExoticComponent } from 'react';
import { PolyForwardComponent } from '@koobiq/react-core';
import type { ReactElement } from 'react';
import type { ReactNode } from 'react';
import { RefAttributes } from 'react';
import type { TransitionProps } from 'react-transition-group/Transition';

// Warning: (ae-forgotten-export) The symbol "CompoundedComponent" needs to be exported by the entry point index.d.ts
//
// @public
export const Accordion: CompoundedComponent;

// @public (undocumented)
export const AccordionComponent: ForwardRefExoticComponent<
  Omit<AccordionProps, 'ref'> & RefAttributes<HTMLDivElement>
>;

// @public (undocumented)
export const AccordionGroup: ForwardRefExoticComponent<
  Omit<AccordionGroupProps, 'ref'> & RefAttributes<HTMLDivElement>
>;

// @public (undocumented)
export type AccordionGroupProps = ExtendableComponentPropsWithRef<
  DisclosureGroupProps & {
    children?: ReactNode;
  },
  'div'
>;

// @public (undocumented)
export type AccordionGroupRef = ComponentRef<'div'>;

// @public (undocumented)
export const AccordionGroupStateContext: Context<DisclosureGroupState | null>;

// @public (undocumented)
export type AccordionProps = ExtendableComponentPropsWithRef<
  AriaDisclosureProps & {
    children?: ReactNode;
  },
  'div'
>;

// @public (undocumented)
export type AccordionRef = ComponentRef<'div'>;

// @public (undocumented)
export const AccordionStateContext: Context<DisclosureState>;

// (No @packageDocumentation comment for this package)
```
