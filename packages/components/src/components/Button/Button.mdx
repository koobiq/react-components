...
You can add an icon using the `startIcon` and `endIcon` prop.
...
Add a progress indicator to a button by using the `progress` prop.

<Story of={Stories.Progress} />

## Disabled

A button can be disabled by passing the `disabled` prop.
