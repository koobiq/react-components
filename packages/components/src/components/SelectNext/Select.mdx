...
- `Select.ItemText` for text and captions. Set `showOverflowTooltip` to show text that doesn't fit in a tooltip.
...
there are at least that many options. It works together with `isSearchable`. The default is `0`,
...
an option, and set `textValue` on the item so it keeps a plain-text label for search and
