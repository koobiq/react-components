...
<Alert title="Note">
  ...
  [`highlight.js@^11`](https://github.com/highlightjs/highlight.js/tree/stable-11)
...
import {
  CodeBlock,
  CodeBlockProvider,
} from '@koobiq/react-components/code-block';
```
...
reduce bundle size, wrap the app (or a part of it) with `CodeBlockProvider` and specify only the
languages you need:
...
register `xml` to enable HTML syntax highlighting. Pass a stable (module-level or memoized) `highlightConfig`
