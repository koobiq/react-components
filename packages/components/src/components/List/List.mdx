...
List consists of a container element, with a list of items or sections inside. Users can select one or more items by clicking, tapping, or navigating with the keyboard.
...
- `List.ItemAddon` — a component to show addon in an item.
...
List items can be composed with helper components:

- `List.ItemText` for text and captions.
- `List.ItemAddon` for icons, badges, or other secondary content.

List items also have layout props, such as `align`, which sets vertical alignment. Use `align="start"` for captions or multi-line content.

<Alert>
  If `List.Item` does not have plain text content in `children`, pass
...
Truncated text inside `List.ItemText` automatically shows its full content when
hovered. The main text and caption are measured independently, and the tooltip
appears above the item. Wrapped text that fits horizontally does not show a
tooltip.

Set `hideTooltip` on `List.ItemText` to disable both tooltips. Plain item content
does not receive an overflow tooltip. The same prop is available on components
that reuse this slot, such as `Select.ItemText`, `Menu.ItemText`, and
