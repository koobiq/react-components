...
Tables can be made resizable. To enable resizing, set the `isResizable` prop to `true`.
And to make a column resizable, set the `allowsResizing` prop to `true` on the column.
