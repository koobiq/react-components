...
Set `showOverflowTooltip` on `DropdownMenu.ItemText` to show text that doesn't fit in a tooltip.
On an item that opens a submenu the tooltip is placed above the item.
...
accessibility. Pass the query in `dependencies` of `DropdownMenu.Content` — otherwise the highlight
does not update.
