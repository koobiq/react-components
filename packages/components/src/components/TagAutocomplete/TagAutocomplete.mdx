...
The `TagAutocomplete.ListItem` provides the suggestion key and `textValue` used
...
matched part of a suggestion, and set `textValue` on the item so it keeps a plain-text label for
search and accessibility.

The collection caches a rendered item by the identity of its data object, so the suggestions have
to be new objects when the query changes — a constant array keeps the old highlight.
...
Prepend an extra item built from the input value and handle it in `onAdd`:

<Story of={Stories.CreateOption} />
...
The `TagAutocomplete` is a full form field on par with `TagInput`: label, caption,
validation, and label placement work the same way. When wrapped in a `<Form>`,
it also inherits the shared field settings from the form context.
...
The cleaner button is shown by default. Pressing it removes all tags, clears
the text input, calls `onClear`, and returns focus to the input. Pass
`hideClearButton` to hide it.

<Story of={Stories.HideClearButton} />
